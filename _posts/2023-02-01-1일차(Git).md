...
---
# 1. 개요
Aivle School OT 이후 1일차 주제는 Git 이었다. 강사로는 생활코딩으로 유명하신 이고잉님이 강사로 오셨다. Git은 이전부터 사용은 해왔지만, 제대로 배워서 알고 사용한 것은 아니었기에 좋은 강의를 통해 기초를 다질 수 있었다. 강의는 VS Code를 활용하여 GUI 형태로 Git을 사용하는 방법을 알려주셨고, Git의 버전이 등록되는 이론적인 부분을 설명해주셨기 때문에 기존에 알고 있던 지식과 연관하여 이해할 수 있었다.
